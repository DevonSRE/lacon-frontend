...
const navItems: NavItem[] = [
  {
    ...
    name: "Home",
    path: "/dashboard",
    roles: ["PLATFORM ADMIN", "ADMIN", "DECONGESTION UNIT HEAD", "LACON LAWYER", "PRO BONO LAWYER", "PDSS", "PRO_BONO_LAWYER", "LAWYER", "DIRECTOR GENERAL", "ZONAL DIRECTOR", "STATE COORDINATOR", "CENTRE COORDINATOR", "CIVIL JUSTICE DEPT. HEAD", "CRIMINAL JUSTICE DEPT. HEAD", "OSCAR UNIT HEAD", "PREROGATIVE OF MERCY UNIT HEAD", "DIO", "PARALEGAL"],
  },
  {
    icon: <Icons.casesIcon />,
    name: "Cases",
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  console.log('Sidebar state:', { isExpanded, isMobileOpen, isHovered }); // Debug log

  // Memoize filtered navigation items for better performance
  const filteredNavItems = useMemo(() => {
    console.log('User role:', user?.role); // Debug log
    console.log('NavItems before filtering:', navItems); // Debug log

    const filtered = navItems
      .filter((item) => {
        const hasAccess = !item.roles || (user?.role && item.roles.includes(user.role as ROLES));
        console.log(`Item "${item.name}" access:`, hasAccess, 'Required roles:', item.roles); // Debug log
        return hasAccess;
      })
      .map((item) => {
        if (item.subItems) {
    ...
    console.log('Filtered nav items:', filtered); // Debug log
    return filtered;
  }, [user?.role]);
