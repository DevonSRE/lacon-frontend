...
import { caseDetailsSchema, decongestionCaseDetails, FormDataDEcongestionCase, personalDecongestionInfoSchema, personalInfoSchema } from '../../probunoLawyers/server/probonoSchema';
...
export default function DecongestionForm({ currentStep = 1, setCurrentStep = () => { } }: DecongestionFormProps) {
    ...
    const [formData, setFormData] = useState<FormDataDEcongestionCase>({
        // Step 1 - Personal Info (From first image)
        first_name: '', // Name of Interviewer
        middle_name: '',
        last_name: '',
        gender: '',
        age: 0,
        last_address: '', // Changed from permanent_address
        marital_status: '',
        has_lawyer: '', // Do you have a lawyer?
        legal_aid_representation: '', // Do you wish to be represented by Legal Aid Council?
        custodial_legal_visited: '', // Custodial legal Visited
        date_of_visit: '', // Date of Visit (dropdown)
        date_of_visit_specific: '', // Date of Visit (specific date)

        // Step 2 - Case Details (From second image)
        case_name: '',
        name_of_defendant: '',
        offence_charged_description: '', // First offence charged (textarea)
        offence_charged: '', // Second offence charged (input)
        charge_number: '', // Changed to string as it can be text
        court_of_trial: '',
        arrest_date: '', // Date of Arrest/Complaint
        arraignment_date: '', // Date of Arraignment/Commencement
        remand_date: '', // Date of Remand(if different from commencement)
        last_court_date: '', // Last Date in Court
        next_adjournment: '',
        bail_status: '', // Bail Status(Granted, Denied, Not Perfected, Not Yet Considered)

        // Defendant Personal Info (From second image)
        sex: '', // Sex dropdown
        date_of_birth_age: '', // Date of Birth/Age dropdown
        name_of_relative: '', // Name of Relative dropdown
        relative_phone_number: '', // Relative's Phone number dropdown
        state_of_origin: '', // State of Origin dropdown
        religion: '', // Religion dropdown
        average_monthly_income: '', // Average monthly Income dropdown
        stage_of_case: '', // Stage of case dropdown
        need_interpreter: '', // Need for Interpreter(If Yes, Which language?)
        disability_ailment: '', // Any Disability or Underlying ailment?
        confessional_statement: '', // Any Confessional statement(How Obtained/Circumstances Thereof)
    });
                    ...
                    <div className="w-full mb-10">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    ...
                                    <div>
                                        <SelectField
                                            ...
                                            required
                                            value={formData.has_lawyer}
                                            onValueChange={(value) => handleSelectChange(value, 'has_lawyer')}
                                            error={!!errors.has_lawyer}
                                            errorMessage={errors.has_lawyer}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    <div>
                                        <SelectField
                                            ...
                                            required
                                            value={formData.legal_aid_representation}
                                            onValueChange={(value) => handleSelectChange(value, 'legal_aid_representation')}
                                            error={!!errors.legal_aid_representation}
                                            errorMessage={errors.legal_aid_representation}
                                        />
                                    </div>
                                    <div>
                                        <InputField
                                            type="text"
                                            name="custodial_legal_visited"
                                            label='Custodial legal Visited'
                                            placeholder="-------------------------"
                                            value={formData.custodial_legal_visited}
                                            onChange={(e) => updateField('custodial_legal_visited', e.target.value)}
                                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                                        />
                                    </div>
                                </div>
                        ...
                        {currentStep === 2 && (
                            <div className="bg-white flex flex-col mt-5">
                                <h2 className="text-xl font-semibold mb-6">Filing A Decongestion Case</h2>
