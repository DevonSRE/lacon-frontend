...
export const createUserSchema = z.object({
    ...
    phone_number: z.string().min(1, "Phone number is required").regex(/^[\+]?[\d\s\-\(\)]{10,}$/, "Please enter a valid phone number"),
}).superRefine((data, ctx) => {
    if ((data.user_type === "LacON Lawyer" || data.user_type === "Paralegal") && !data.designation) {
        ctx.addIssue({
    ...
    const needsState = data.user_type === "State Coordinator" ||
